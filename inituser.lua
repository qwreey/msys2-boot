local BOOTROOT = os.getenv("BOOTROOT")
...
local config = {} do
	local file,err = io.open(BOOTROOT.."\\boot\\user.conf","r")
...
-- write nsswitch conf
do
	local file,err = io.open(BOOTROOT.."\\etc\\nsswitch.conf","w")
	if not file then
...
-- write post install
do
	local file,err = io.open(BOOTROOT.."\\etc\\post-install\\00-additional-path.post","w")
	if not file then
		error("failed to open /etc/post-install/00-additional-path.post\n"..err)
	end
	file:write('[ -n "$BOOTAPATH" ] && export PATH="$BOOTAPATH:$PATH"\n')
	file:close()
end

do -- write passwd
	local this_passwd do
		local file,err = io.popen(BOOTROOT.."\\usr\\bin\\mkpasswd -l -u Administrator -c")
	...
	-- create passwd file
	local file,err = io.open(BOOTROOT.."\\etc\\passwd","w")
...
do -- write group
	local this_group do
		local file,err = io.popen(BOOTROOT.."\\usr\\bin\\mkgroup.exe -c -g Administrators")
		if not file then error("failed to execute mkgroup -c\n"..err) end
		this_group = file:read("*a")
		file:close()
	end

	local file,err = io.open(BOOTROOT.."\\etc\\group","w")
